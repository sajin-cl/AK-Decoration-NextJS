...
const Banner = ({ data }) => {
    ...
    return (
        <section id='service-detail-banner' className='relative w-full overflow-x-hidden min-h-screen'>
            <div className=' image-banner-container w-full'>
                <Image
                    src={image}
                    alt={title}
                    fill
            ...
            {/* Absolute content */}
            <div className="absolute bottom-50 left-5 md:bottom-20 md:left-10 z-40">
                <p className='sub-title font-semibold text-white text-[12px]'>{subtitle}</p>
                <h1 className='title font-bodoni text-white text-[49px]'>{title}</h1>
